...
actor {
  ...
  module ProductMaster {
    public func compare(p1 : ProductMaster, p2 : ProductMaster) : Order.Order {
      Nat.compare(p1.id, p2.id);
    };
  };
  ...
  let products = Map.empty<Nat, ProductMaster>();
  ...
  // New Function: Get specific project (query)
  public query ({ caller }) func getProject(id : Nat) : async ?Project {
    projects.get(id);
  };

  // New Function: Update existing project (shared)
  public shared ({ caller }) func updateProject(id : Nat, clientName : Text, systemType : { #onGrid; #offGrid; #hybrid }, installationType : { #rccRooftop; #sheetMetal; #groundMount; #other }, systemSizeKW : Float, batteryCapacityKWh : Float) : async () {
  ...
  public shared ({ caller }) func generateMOQ(projectId : Nat) : async () {
    ...
    let toDelete = moqItems.values().toArray().filter(func(m : MOQItem) : Bool { if (m.projectId == projectId) { true } else { false } });
    for (item in toDelete.vals()) {
  ...
  public shared ({ caller }) func createProductMaster(category : Text, brand : Text, productType : Text, capacity : Text, voltage : Text, pricePerUnit : Float, unit : Text, efficiency : Float, warrantyYears : Nat, isActive : Bool) : async Nat {
  ...
  public shared ({ caller }) func updateProductMaster(id : Nat, category : Text, brand : Text, productType : Text, capacity : Text, voltage : Text, pricePerUnit : Float, unit : Text, efficiency : Float, warrantyYears : Nat, isActive : Bool) : async () {
    switch (products.get(id)) {
      case (null) { Runtime.trap("Product does not exist") };
      case (?product) {
        let updatedProduct : ProductMaster = {
          id;
          category;
          brand;
          productType;
  ...
  public shared ({ caller }) func deleteProductMaster(id : Nat) : async () {
  ...
  public query ({ caller }) func listProductMaster() : async [ProductMaster] {
    products.values().toArray().sort();
  };

  public query ({ caller }) func listProductMasterByCategory(category : Text) : async [ProductMaster] {
    products.values().toArray().filter(func(p) { p.category == category and p.isActive });
  };

  public shared ({ caller }) func generateMOQWithProducts(projectId : Nat, panelProductId : ?Nat, inverterProductId : ?Nat, batteryProductId : ?Nat, cableProductId : ?Nat, structureProductId : ?Nat) : async () {
    let project = switch (projects.get(projectId)) {
      case (null) { Runtime.trap("Project not found") };
      case (?p) { p };
    };
    ...
    let panelPrice = switch (panelProductId) {
      case (null) { 22000.0 };
      case (?id) {
        switch (products.get(id)) {
          case (null) { 22000.0 };
          case (?p) {
            if (p.isActive and p.category == "Solar Panel") {
              p.pricePerUnit;
            } else { 22000.0 };
          };
        };
    ...
    let inverterPriceFinal = switch (inverterProductId) {
      case (null) { inverterPrice };
      case (?id) {
        switch (products.get(id)) {
          case (null) { inverterPrice };
          case (?p) {
            if (p.isActive and p.category == "Inverter") {
              p.pricePerUnit;
            } else { inverterPrice };
          };
        };
    ...
    let batteryPrice = switch (batteryProductId) {
      case (null) { 14000.0 };
      case (?id) {
        switch (products.get(id)) {
          case (null) { 14000.0 };
          case (?p) {
            if (p.isActive and p.category == "Battery") {
              p.pricePerUnit;
            } else { 14000.0 };
          };
        };
    ...
    let cablePrice = switch (cableProductId) {
      case (null) { 35.0 };
      case (?id) {
        switch (products.get(id)) {
          case (null) { 35.0 };
          case (?p) {
            if (p.isActive and p.category == "Cable") {
              p.pricePerUnit;
            } else { 35.0 };
          };
        };
    ...
    let structurePrice = switch (structureProductId) {
      case (null) { 4000.0 };
      case (?id) {
        switch (products.get(id)) {
          case (null) { 4000.0 };
          case (?p) {
            if (p.isActive and p.category == "Structure") {
              p.pricePerUnit;
            } else { 4000.0 };
          };
        };
    ...
    insertItem("Solar PV", "Solar Panel 540 Wp Mono PERC", panelQty, "Nos", panelPrice);
    insertItem("Solar PV", "Solar Inverter", 1.0, "Nos", inverterPriceFinal);

    if (needsBattery) {
      insertItem("Battery Bank", "VRLA Battery 200Ah 12V", battQty, "Nos", batteryPrice);
      insertItem("Battery Bank", "Battery Cable Set", Float.ceil(battQty / 2.0), "Set", 500.0);
    };
    ...
    insertItem("Cabling", "DC Solar Cable 6sqmm Red", Float.ceil(dcCableM / 2.0), "Mtr", cablePrice);
    insertItem("Cabling", "DC Solar Cable 6sqmm Black", Float.ceil(dcCableM / 2.0), "Mtr", cablePrice);
    insertItem("Cabling", "AC Power Cable 4sqmm", 30.0, "Mtr", 28.0);
    insertItem("Cabling", "Earthing Cable 16sqmm", 20.0, "Mtr", 45.0);
    insertItem("Cabling", "Conduit Pipe 25mm", Float.ceil(dcCableM / 3.0), "Nos", 85.0);
    insertItem("Cabling", "Cable Tie Pack 100pcs", 3.0, "Pkt", 120.0);
    ...
    switch (project.installationType) {
      case (#rccRooftop) { insertItem("Mounting Structure", "MS Aluminium Rooftop Structure", panelQty, "Nos", structurePrice) };
      case (#sheetMetal) { insertItem("Mounting Structure", "GI Roof Hook MS Channel Structure", panelQty, "Nos", 3500.0) };
      case (#groundMount) { insertItem("Mounting Structure", "MS Ground Mount Structure", panelQty, "Nos", 6500.0) };
      case (#other) { insertItem("Mounting Structure", "Aluminium Elevated Structure", panelQty, "Nos", 8000.0) };
    };
